...
std::string AsString::serialize(const uprotocol::v1::UUID& uuid) {
	...
	ss << std::hex << std::setfill('0') << std::setw(LEN_32_BITS_IN_HEX)
	   << ((unix_ts_ms >> LEN_LSB_IN_HEX*LEN_HEX_TO_BIT) & MASK_32_BITS)  // First 32 bits of timestamp
...
uprotocol::v1::UUID AsString::deserialize(const std::string& str) {
	...
	if (str.length() != TOTAL_UUID_LENGTH || str[POS_FIRST_SEPARATOR] != '-' || str[POS_SECOND_SEPARATOR] != '-' ||
	    str[POS_THIRD_SEPARATOR] != '-' || str[POS_FOURTH_SEPARATOR] != '-') {
	...
	try {
		...
		unix_ts_ms = std::stoull(str.substr(0, LEN_MSB_IN_HEX), nullptr, HEX_BASE) << LEN_LSB_IN_HEX*LEN_HEX_TO_BIT;
		unix_ts_ms |= std::stoull(str.substr(POS_FIRST_SEPARATOR+1, LEN_LSB_IN_HEX), nullptr, HEX_BASE);
		...
		uint16_t var_randb = static_cast<uint16_t>(
		    std::stoul(str.substr(POS_THIRD_SEPARATOR +1, LEN_VARR_IN_HEX), nullptr, HEX_BASE));
...
std::vector<uint8_t> AsBytes::serialize(const v1::UUID& uuid) {
	...
	uint32_t msb_high = htonl(static_cast<uint32_t>(uuid.msb() >> LEN_UINT64_IN_BIT/2));
	uint32_t msb_low  = htonl(static_cast<uint32_t>(uuid.msb() & MASK_32_BITS));
	uint32_t lsb_high = htonl(static_cast<uint32_t>(uuid.lsb() >> LEN_UINT64_IN_BIT/2));
...
v1::UUID AsBytes::deserialize(const std::vector<uint8_t>& bytes) {
	...
	uint64_t msb =
	    (static_cast<uint64_t>(ntohl(msb_high)) << LEN_UINT64_IN_BIT/2) | ntohl(msb_low);
	uint64_t lsb =
	    (static_cast<uint64_t>(ntohl(lsb_high)) << LEN_UINT64_IN_BIT/2) | ntohl(lsb_low);
