...
v1::UMessage UMessageBuilder::build() {
	v1::UMessage message;
	...
	*attributes_.mutable_id() = uuidBuilder_->getBuilder().build();
	*message.mutable_attributes() = attributes_;

	return message;
}

v1::UMessage UMessageBuilder::build(builder::Payload&& payload) {
	v1::UMessage message;

	*attributes_.mutable_id() = uuidBuilder_->getBuilder().build();
	*message.mutable_attributes() = attributes_;
	auto [payloadData, payloadFormat] = payload.build();
	if (expectedPayloadFormat_.has_value()) {
