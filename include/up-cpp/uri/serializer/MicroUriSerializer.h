...
public:
    /**
     * Serialize a UUri into a vector<uint8_t> following the Micro-URI
     * specifications.
     *
    ...
    [[nodiscard]] static auto serialize(
            const uprotocol::v1::UUri& u_uri) -> std::vector<uint8_t>;

    /**
     * Serialize a UAuthority into a vector<uint8_t> following the Micro-URI
     * specifications.
     *
     ...
     */
    [[nodiscard]] static auto serialize(
            const uprotocol::v1::UAuthority& u_auth)
        -> std::pair<AuthorityType, std::vector<uint8_t>>;
     ...
     *
     * @return Returns an UUri data object from the serialized format of a
     *         micro URI.
     */
    [[nodiscard]] static auto deserialize(
            std::vector<uint8_t> const& micro_uri) -> uprotocol::v1::UUri;
...
private:
     ...
     * @param type 
     *
     * @return AuthorityType, with AuthorityType::Invalid indicating an
     *         unsupported type
     */
     ...
     * @return 
     */
    [[nodiscard]] static auto checkMicroUriSize(
            std::size_t size, AuthorityType type) -> bool;

    /**
     * Get UAthority from the address and type
     * @param addr 
     * @param type 
     * @return uprotocol::v1::UAuthority if address is not valid UAuthority
     *         return empty
     */
    [[nodiscard]] static auto getUauthority(
            const std::vector<uint8_t> &addr,
            AuthorityType type) -> uprotocol::v1::UAuthority;
...
private:
     ...
     * @return 
     */
    [[nodiscard]] [[maybe_unused]] inline auto isMicroForm(
