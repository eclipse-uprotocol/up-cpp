...
public:
     ...
     *
     * @return Returns the String format of the supplied UUri that can be used
   ...
   /**
     ...
     *
     * @return Returns the String representation of the  Resource in the
     *         uProtocol URI.
     */
    [[nodiscard]] static auto buildResourcePartOfUri(
            const v1::UResource& resource) -> std::string;

    /**
     * Create the service part of the uProtocol URI from an  software
     * entity object.
     *
     * @param use Software Entity representing a service or an application.
     *
     * @return Returns the String representation of the  Software Entity in
     *         the uProtocol URI.
     */
    [[nodiscard]] static auto buildSoftwareEntityPartOfUri(
            const v1::UEntity& use) -> std::string;
     ...
     */
    [[nodiscard]] static auto buildAuthorityPartOfUri(
            const v1::UAuthority& u_authority) -> std::string;
...
private:
    ...
    [[nodiscard]] static auto parseUResource(
            const std::string& resource_string) -> v1::UResource;

    /**
     * Static factory method for creating a UEntity using a string that
     ...
     */
    [[nodiscard]] static auto parseUEntity(
            const std::string &entity,
            const std::string &version) -> v1::UEntity;

    /**
     * Static factory method for creating a UUri using a vector of strings that
     * contains the Local UUri information.
     *
     * @param uri_parts Vector of strings that contains the Local UUri
     *                  information.
     *
     * @return Returns a v1::UUri object.
     */
    [[nodiscard]] static auto parseLocalUUri(
            const std::vector<std::string>& uri_parts) -> v1::UUri;

    /**
     * Static factory method for creating a UUri using a vector of strings that
     * contains the Remote UUri information.
     *
     * @param uriParts Vector of strings that contains the Remote UUri
     *                 information.
     *
     * @return Returns a UUri object.
    */
    [[nodiscard]] static auto parseRemoteUUri(
            const std::vector<std::string>& uri_parts) -> v1::UUri;
     ...
     * @return 
     */
    [[nodiscard]] static auto inline isLocality(
            const std::string_view &uri) -> bool;
    /**
     ...
     * @return 
     */
    [[nodiscard]] static auto inline getFirstNotEmpty(
            const std::vector<std::string> &uri_parts) -> uint64_t ;
}; // class LongUriSerializer
